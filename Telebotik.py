...
def start(message):
    ...
    bot.send_message(message.chat.id,"Привет {0} \U0001F44B\! Вы играете в игру поле чудес\. По ходу игры вы зарабатываете очки\,которые можете потратить на разные вещи\. А сейчас вращайте барабан\! ".format(message.from_user.first_name),parse_mode='MarkdownV2',reply_markup=markup)
...
def my_answer(message):
    ...
    if message.text == "Вращать барабан \U0001F579":
        if base.read_base(message.from_user.id) != 'no':
            action = dict_user[message.from_user.id].action(str(random.randint(1,9)))
        ...
        if base.read_base(message.from_user.id) != 'no':
            chance = dict_user[message.from_user.id].get_chanse()
            kf = KolesoFortuni()
            if chance > 1:
                ...
                dict_user[message.from_user.id].set_chanse(kf.get_chanse())
            else:
    
                bot.send_message(message.chat.id,'У вас пока нет шансов для колеса фортуны')
